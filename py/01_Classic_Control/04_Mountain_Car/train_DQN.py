...
def arguments() :
    parser = argparse.ArgumentParser(
        ...
        prog='Gym - MountainCar 训练脚本',
        description='在默认环境下、使用深度 Q 网络（DQN）训练智能体操作 MountainCar', 
        epilog='\r\n'.join([
            '运行环境: python3', 
            '运行示例: python py/01_Classic_Control/04_Mountain_Car/train_DQN.py'
        ])
    )
...
def train(writer : SummaryWriter, targs : TrainArgs, epoch) :
    ...
    raw_obs = targs.reset_env()         # 重置环境（在 MountainCar 环境中，这个初始状态就是观测空间，它包含了关于 MountainCar 状态的数组）
