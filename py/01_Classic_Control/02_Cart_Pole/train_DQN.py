...
# -----------------------------------------------
# 经典控制： CartPole （杂技机器人）
#   CartPole 是一个双节摆问题，目标是用最少的步骤使得摆的末端达到一定高度。
# 
# 相关文档：
...
def arguments() :
    parser = argparse.ArgumentParser(
        ...
        epilog='\r\n'.join([
            '运行环境: python3', 
            '运行示例: python py/01_Classic_Control/01_CartPole/train_DQN.py'
        ])
    )
    ...
    parser.add_argument('-t', '--tensor_logs', dest='tensor_logs', type=str, default=TENSOR_PATH, help='TensorBoardX 日志目录')
...
def train(writer : SummaryWriter, targs : TrainArgs, epoch) :
    ...
    :return: None
    '''
    raw_obs = targs.env.reset()         # 重置环境（在CartPole环境中，这个初始状态就是观测空间，它包含了关于 CartPole 状态的数组，例如两个连杆的角度和角速度等）
