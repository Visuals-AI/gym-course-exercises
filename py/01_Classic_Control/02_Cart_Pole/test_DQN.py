...
def arguments() :
    parser = argparse.ArgumentParser(
        ...
        epilog='\r\n'.join([
            '运行环境: python3', 
            '运行示例: python py/01_Classic_Control/01_CartPole/test_DQN.py'
        ])
    )
...
def test(targs : TrainArgs, epoch) :
    ...
    # CartPole 问题的 v1 版本要求在 500 步内完成
    ...
    if step_counter < ACROBOT_V1_MAX_STEP :
        log.debug(f"[第 {epoch} 回合] 智能体在第 {step_counter} 步完成 CartPole 挑战")
        is_ok = True
    else :
        log.debug(f"[第 {epoch} 回合] 智能体未能在 {ACROBOT_V1_MAX_STEP} 步内完成 CartPole 挑战")
        pass
    return is_ok
